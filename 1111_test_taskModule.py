# 1111_test_taskModule.py
import os
import json

"""
任务指令划分与任务地址对齐
- 加载网络结构配置文件，识别各层参数（卷积、池化等）
- 读取算子库中的二进制指令配置，匹配网络层与对应算子
- 按层划分任务：卷积层按输出通道数划分（每10个通道一个任务），池化层固定1个任务
...
def generate_original_task_file(network, operators, output_path):
    ...
    for layer_idx, layer in enumerate(network, 1):
        ...
        if layer["operator"] == "Conv":
            ...
            task_count = (total_out + 9) // 10  # 计算任务数（如64→7次）
            # 打印每层任务划分信息
...
def main(network_path, library_path, original_output, aligned_output):
...
if __name__ == "__main__":
    ...
    LIBRARY_PATH = "Op_Library"  # 算子库目录
    ORIGINAL_OUTPUT = "新版总任务指令配置.txt"  # 原始任务指令输出文件
    ALIGNED_OUTPUT = "新版总任务指令配置_per_task_addr256k.txt"  # 地址对齐任务指令输出文件
    # =====================================

    main(NETWORK_PATH, LIBRARY_PATH, ORIGINAL_OUTPUT, ALIGNED_OUTPUT)
