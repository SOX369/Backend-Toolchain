...
# 状态指示灯颜色调整，适配白底
COLOR_WAITING = "gray80"  # 浅灰色：待执行 (白底上用浅灰)
...
class AIChipToolchainApp(ctk.CTk):
    ...
    def setup_sidebar(self):
        ...
        self.desc_label = ctk.CTkLabel(self.sidebar_frame, text="End-to-End Compiler Pipeline",
                                       text_color="gray40", font=ctk.CTkFont(size=12))
        ...
        # 分割线颜色变浅，适应白底
        self.separator = ctk.CTkFrame(self.sidebar_frame, height=2, fg_color="gray85")
    ...
    def create_progress_panel(self, row):
        ...
        for name in self.stage_names:
            ...
            # 文本标签颜色加深 (gray20) 以在浅色模式下清晰显示
            lbl = ctk.CTkLabel(item_frame, text=name, text_color="gray20", font=ctk.CTkFont(size=12))
            lbl.pack(side="left")
    ...
    def setup_main_area(self):
        ...
        # 按钮颜色微调，使其在浅色背景上更协调
        ctk.CTkButton(header, text="Clear Logs", width=80, height=28, fg_color="gray60",
                      command=self.clear_log).pack(side="right")

        # 终端文本框 (CustomTkinter 在浅色模式下会自动使用浅灰/白底和黑色文字)
